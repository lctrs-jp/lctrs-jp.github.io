...
int load(char *filename)//ファイルの読み込み
{
 ...
 FILE *fp;
  ...
  sort(year,month,day,item.ok,item.unifixed);
...
int okq(char *okx[]){
	...
	for(i=0;i<3;i++){
		if(okx[i] == 'o') item.ok++;
		if(okx[i] == '?') item.unfixed++;
	}

}


int sort(int year,int month,int day,int ok,int unifixed){
...
void print(){
	...
	m = (n / 100) % 100;
	y = n / 10000;
	printf("%4d/%2d/%2d\t%c\t%c\t%c\n",y,m,d,item[i].alice,item[i].bob,item[i].carol);
	}
	printf("---------------------------------");
