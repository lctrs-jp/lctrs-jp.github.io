...
int main(int argc,char *argv[]){
  ...
  ATTEND item[MAX+1]; //MAXを増やして、バブルソートに備える。
  int size;
...
void print(ATTEND data[]){
  ...
  for(int k=0;data[k].date != 0 && k<MAX;k++){
    if(data[k].ok<data[k+1].unfixed){
      data[MAX]=data[k];
      data[k]=data[k+1];
      data[k+1]=data[MAX];
    }else if(data[k].ok==data[k+1] && data[k].unfixed<data[k+1].unfixed){
      data[MAX]=data[k];
      data[k]=data[k+1];
      data[k+1]=data[MAX];
    }
  ...
  for(int j=0;data[j].date != 0;j++)
    printf("%d: [ok:%d, ?:%d]",j,data[j].ok,data[j].unfixed);//順位を加える
