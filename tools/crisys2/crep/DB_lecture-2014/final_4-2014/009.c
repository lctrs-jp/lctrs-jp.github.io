...
void sortdata(struct attendance item[]);
...
int main(int argc, char *argv[])
{
  ...
  for(i = 0; i < item[i].date[0] != '\0'; i++) {
    if(item[i].alice == 'o') item[i].ok += 1;
    else if(item[i].alice == '?') item[i].unfixed += 1;
    if(item[i].bob == 'o') item[i].ok += 1;
    else if(item[i].bob == '?') item[i].unfixed += 1;
    if(item[i].carol == 'o') item[i].ok += 1;
    else if(item[i].carol == '?') item[i].unfixed += 1;
  }
...
{
  int i, j;
  struct attendance work;

  for(i = 0; i < MAX; i++)
    for(j = MAX - 1; j > i; j--) {
      if(item[j].date[0] == '\0') continue;
      if(item[i].ok < item[j].ok) {
        work = item[i];
        item[i] = item[j];
        item[j] = work;
      }
      else if(item[i].ok == item[j].ok) {
        if(item[i].unfixed < item[j].unfixed) {
	  work = item[i];
	  item[i] = item[j];
	  item[j] = work;
	}
