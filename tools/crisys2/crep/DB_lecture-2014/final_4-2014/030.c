...
int main (int argc, char *argv[])
{
  ...
  int i, t, count = 0,day,month,year,n,rank;
  ...
  for (i = 0;i < argc-1; i++){
	...
	for (t = 0, p = item; t < count && strcmp(p->date,date) != 0; t++, p++);
  ...
  for (i = 0; i < count - 1; i++){
    p = item;
    for (t = i; t < count - 1; t++, p++){
      if (p->ok < (p+1)->ok){
	esc = *p;
	*p = *(p+1);
	*(p+1) = esc;
      }	else if(p->ok == (p+1)->ok && p->unfixed < (p+1)->unfixed){
	esc = *p;
	*p = *(p+1);
	*(p+1) = esc;
      }
  ...
  for (i = 0, p = item; i < count; i++,p++) {
    ...
    printf("%2d: %4d/%2d/%2d [ok:%d, ?:%d]\n",rank,year,month,day,p->ok,p->unfixed);
