...
void sort(ATT item[MAX], int count)
{
  int a, b;
  ATT t;
  for(a = 1; a < count; a++)
  ...
  for(a = 1; a < count; a++)
    for(b = count - 1; b >= a; b--){
      if( item[b-1].ok == item[b].ok &&  item[b-1].unfixed < item[b].unfixed){
	t = item[b-1];
	item[b-1] = item[b];
	item[b] = t;
      }
...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<size;i++){
    ...
    printf("%d: %4d/%2d/%2d [ok:%d, ?:%d]\n", c, year, month, day, item[i].ok, item[i].unfixed);

    d++;
    if(item[i].ok != item[i+1].ok ||
       item[i].unfixed != item[i+1].unfixed)
      c = d;
