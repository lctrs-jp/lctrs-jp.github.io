...
int split(char buf[], char *out[]){
  int i, k, rt = 0;//buf[i]を順次調べる,rtは非スペース群の個数(rt個目),
  char sep[80][40];//bufを分割したものそのもの
  for(i = 0; i != 80; i++) for(k = 0; k != 40; k++) sep[i][k] = '\0';//sep初期化
  for(i = 0; buf[i] == ' '; i++);//最初の非スペースに飛ぶ
  while(buf[i] != '\0'){
    if(buf[i] == ' '){
      for(; buf[i] ==' '; i++);//次の非スペースに飛ぶ
    }//spaceのとき
    else{
      k = i;//kは非スペース群の先頭
      for(; buf[i] !=' ' && buf[i] != '\0'; i++)sep[rt][i - k] = buf[i];
//sepに代入しつつ次のスペースorEOFに飛ぶ
      out[rt] = sep[rt];
      rt++;
    }
  }
