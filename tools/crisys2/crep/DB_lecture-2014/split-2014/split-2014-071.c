...
int split(char buf[], char *out[]){
  ...
  for(n = 0; buf[n] != '\0' ; n++)
    {
      if(buf[n] != ' '&&n < 1)/*一文字目だけ見て、スペースでないならoutにアドレスを*/
	{
	  out[a] = &buf[n];
	  for(b = n ; buf[b] != ' '; b++)
	    {
      ...
      else if(buf[n] == ' '&&n<1)/*一文字目が空白なら無視して次の文字へ*/
	{
	  n++;
	}

      else if(buf[n] == ' '&&n>0)/*一文字目以降も空白なら無視*/
	{
	  n++;
	}

      else if(buf[n] != ' '&&n > 0)/*二文字目以降を見る*/
	{
	  if(buf[n-1] != ' ')/*その文字の一文字前の文字が空白でないなら、その文字は一塊の中に入るので分割せずに次へ*/
	    { 
	      n++;
	    }
      
	  else if(buf[n-1] == ' ')/*その文字の一文字前の文字が空白ならその文字が次の塊の先頭といえる*/
	    {
	      out[a] = &buf[n];
	      for(b=n; buf[b] != ' '; b++)
		{
